...
_z() {
    ...
    if [ "$1" = "--add" ]; then
        ...
        for exclude in "${_Z_EXCLUDE_DIRS[@]}"; do
            # case "$*" in "$exclude"*) return;; esac               ## MODIFIED BY JMC 09/13/2015
            [[ "$*" == $exclude* ]] && return
        done
    ...
    # tab completion
        ...
        cd="$(while read line; do
            ...
            {
                n = split($1, broken, "/")
                treshhold = length($1) - length(broken[n])
...
# if compctl >/dev/null 2>&1; then                                      ## MODIFIED BY JMC 09/13/2015
...
# elif complete >/dev/null 2>&1; then                                   ## MODIFIED BY JMC 09/13/2015
